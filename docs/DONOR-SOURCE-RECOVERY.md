...
| `release/v2.10.0-submission-inbox` reconstructed Worker | Deferred donor | The supplied reconstruction script produced a checksum-verified archive with real Worker source, migrations, schemas, forms, tests, and Wrangler configuration. It is not integrated yet because it targets `<3.0.0` and lacks a public-registry lockfile. |
