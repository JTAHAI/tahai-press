...
In progress. `npm run verify:ga` now produces the authoritative `.artifacts/ga-gate-report.json` from a clean installation. It installs the pinned Playwright engines, then verifies the root suite, build, accessibility, deployment, performance, reader, media, smoke, release proof, security, production dependency audit, themes, evidence, migration rollback, transfer verification/release packages, Worker, collaboration, and Chromium/Firefox/WebKit matrix. The 2026-08-15 run passed all 24 then-defined gates; the expanded gate is not represented as a fresh GA pass until its browser matrix completes cleanly. Expanded viewport/theme/print/offline/recovery scenarios and the remaining evidence matrices remain open release gates.
