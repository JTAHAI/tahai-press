...
- Active source identity is `3.0.3`; package and lockfile agree.
...
In progress. `npm run verify:enterprise` now records twenty publisher-safe release controls: governance, runtime and lockfile contracts, content/CMS/redirect validation, unit tests, media, build and deployment integrity, accessibility, reader experience, performance, source security, theme supply chain, evidence, migration reversibility, recovery, transfer, and release packaging. `npm run verify:ga` remains the authoritative clean-install browser gate. On 2026-08-21, the expanded gate passed all 27 checks, including the 20-control enterprise plane and Chromium, Firefox, and WebKit. Pagefind now uses an explicitly initialized same-origin main-thread instance, retaining local ranking while avoiding browser-specific Worker lifecycle failures. The static release now also emits a deterministic Cloudflare edge-header policy and a public SHA-256 integrity manifest, with a live verifier that proves the deployed metadata, cache policy, core assets, and response headers without credentials. Cloudflare Pages and Worker static-asset releases have distinct, accurate deployment identities. Expanded viewport/theme/print/offline/recovery scenarios and the remaining evidence matrices remain open release gates.
