...
## Operational release artifacts

The quality and production-readiness workflows also upload private operational artifacts generated by the same source commit:

- `.artifacts/media-health.json`;
- `.artifacts/performance-audit.json`;
- `.artifacts/newsroom-health/`;
- `.artifacts/release-proof/`.

These are GitHub Actions artifacts, not public site routes. They provide maintainers with a readable health summary and machine-readable reports without introducing a monitoring vendor or administrative backend.

The Cloudflare build runs media and performance checks before accepting `dist/`. A missing local upload or exceeded performance budget fails the build instead of producing a partially broken or unexpectedly heavy deployment.

See [OPERATIONS.md](OPERATIONS.md).

## Reader Reach deployment behavior

When offline reading is enabled, the normal build emits `/service-worker.js`, `/site.webmanifest`, and `/offline/`. No Cloudflare Worker, route binding, environment secret, or additional deployment command is required. The files are ordinary static Pages assets.

After a release, verify the service worker on the real HTTPS deployment rather than a file URL. Browsers may keep the prior worker briefly while an open tab is active. Closing old tabs or using the browser's application tools can accelerate a test update, but publishers should not add cache-busting scripts or remote service-worker loaders.

Direct-deploy ZIPs and Git-connected builds contain the same generated worker. The service worker caches only same-origin publication pages and assets.
