...
test('the GA gate covers static delivery, portability, optional services, and real browsers', () => {
  ...
  for (const id of ['root-clean-install', 'playwright-browser-install', 'root-test-suite', 'accessibility-audit', 'security-audit', 'official-themes', 'evidence-validation', 'migration-proof', 'transfer-package', 'transfer-verification', 'release-packages', 'worker-tests', 'collaboration-tests', 'real-browser-matrix']) assert.match(source, new RegExp(`id: '${id}'`));
