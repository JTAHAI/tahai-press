...
test('expert crossword collection includes multiple blocked fifteen-by-fifteen editions with long answers', () => {
  const script = fs.readFileSync(scriptFile, 'utf8');
  assert.match(script, /const expertPuzzles = \[/);
  assert.ok((script.match(/difficulty:\s*'expert'/g) || []).length >= 3);
  const grids = [...script.matchAll(/grid:\s*\[([\s\S]*?)\],\s*clueMap:/g)]
    .map((match) => [...match[1].matchAll(/'([#A-Z]+)'/g)].map((row) => row[1]))
    .filter((rows) => rows.length === 15);
  assert.ok(grids.length >= 3);
  for (const rows of grids) {
    assert.equal(rows.every((row) => row.length === 15), true);
    assert.equal(rows.some((row) => row.includes('#')), true);
    assert.equal(rows.some((row) => /[A-Z]{8,}/.test(row)), true);
  }
});
...
test('crossword route and controls are generated as local static assets', () => {
  ...
  assert.match(script, /is-blocked/);
  assert.match(script, /expertPuzzles/);
