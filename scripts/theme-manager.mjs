...
import { activateTheme, applyThemeToSource, exportInstalledTheme, listInstalledThemes, themePaths } from './lib/themes.mjs';
...
http.createServer(async (request, response) => {
  try {
    ...
    if (request.url === '/apply') { const result = applyThemeToSource(data.theme); send(response, 200, page(`Pinned ${result.id} ${result.version} for the next static build.`)); return; }
